...
Status Gemm<float>::Compute(OpKernelContext* context) const {
  ...
  SystolicGemm(static_cast<const SystolicExecutionProvider*>(this->Info().GetExecutionProvider())->GetAcceleratorMode(),
               /*relu= */ false,
               M, N, K,
               A->Data<float>(), B->Data<float>(),
               y_data,
               alpha_, c_data != nullptr ? beta_ : 0,
               trans_A_, trans_B_,
               c_data == nullptr ? nullptr : y_data);
