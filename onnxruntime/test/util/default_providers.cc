...
std::shared_ptr<IExecutionProviderFactory> CreateExecutionProviderFactory_Hwacha(int use_arena, char accelerator_mode = 0);
...
std::unique_ptr<IExecutionProvider> DefaultHwachaExecutionProvider(bool enable_arena) {
#ifdef USE_HWACHA
  return CreateExecutionProviderFactory_Hwacha(enable_arena)->CreateProvider();
#else
  ORT_UNUSED_PARAMETER(enable_arena);
  return nullptr;
#endif
